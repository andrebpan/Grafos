...
//busca em largura
void bfs(Grafo* grafo, Vertice* inicio, bool* visitado) {
    int fila[MAX_VERTICES];  //armazena os vértices a serem visitados
    int frente = 0, tras = 0;

    //marca o vertice inicial como visitado e adiciona na fila
    visitado[inicio->id] = true;
    fila[tras++] = inicio->id;

    while (frente < tras) { //enquanto fila nao for vazia
        //remove o vertice da frente da fila
        int verticeAtualId = fila[frente++];

        //encontra o vertice correspondente no grafo
        Vertice* verticeAtual = encontraVertice(verticeAtualId, grafo);
        if (verticeAtual == NULL) continue;

        //percorre os vertices adjacentes
        Aresta* a = verticeAtual->adjacente;
        while (a != NULL) {
            //encontra o vértice de destino da aresta
            Vertice* adj = encontraVertice(a->verticeDestino, grafo);
            if (adj != NULL && !visitado[adj->id]) {
                visitado[adj->id] = true; //marca como visitado
                fila[tras++] = adj->id;   //adiciona na fila
            }
            a = a->prox;
        }
    }
...
//verificar se o grafo é conexo usando BFS
bool verificarConexo(Grafo* grafo) {
    ...
    //iniciar BFS a partir do primeiro vértice
    bfs(grafo, grafo->vertices, visitado);
